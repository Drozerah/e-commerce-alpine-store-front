 ...
 */
 ...
 const config = {
   ...
   html: {
    src: path.join(SRC_DIR, MAIN_HTML_FILE_NAME),
 ...
 const _open = () => {
   ...
   return src(__filename).pipe(open({uri: uri}))
 }
...
const startExpress = () => {
 ...
 const watch_html = () => watch( config.html.src, html ).on('change', () => log("Watching 'html'"))
